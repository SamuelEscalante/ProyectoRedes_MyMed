...
df = spark.read.csv("/home/vagrant/labSpark/inventariomymed/inventariomymed.csv", header=True, inferSchema=True, sep=";")
...
# Guardar el dataframe como CSV
df.groupBy("year_compra").sum("Ventas_Totales_Year").orderBy("year_compra").write.csv("/home/vagrant/MyMedApp/db/data/tendecias2.csv", header=True)
...
# Guardar el dataframe como CSV
df.groupBy("laboratorio").sum("Ventas_Totales_Year").orderBy("sum(Ventas_Totales_Year)", ascending=False).write.csv("/home/vagrant/MyMedApp/db/data/segmentacion_laboratorio.csv", header=True)

print("=== Segmentación por condición de venta ===")
df.groupBy("condicion_venta").avg("precio_unitario").orderBy("condicion_venta").show()

# Guardar el dataframe como CSV
df.groupBy("condicion_venta").avg("precio_unitario").orderBy("condicion_venta").write.csv("/home/vagrant/MyMedApp/db/data/segmentacion_condicion_venta.csv", header=True)
...
# Guardar el dataframe como CSV
df.withColumn("tiempo_restante_caducidad", col("year_caducidad") - col("year_compra")).groupBy("tiempo_restante_caducidad").count().orderBy("tiempo_restante_caducidad").write.csv("/home/vagrant/MyMedApp/db/data/analisis_caducidad.csv", header=True)
...
# Guardar el dataframe como CSV
df.orderBy("precio_unitario", ascending=False).write.csv("/home/vagrant/MyMedApp/db/data/analisis_precios.csv", header=True)
df.groupBy("laboratorio").avg("precio_unitario").orderBy("avg(precio_unitario)", ascending=False).write.csv("/home/vagrant/MyMedApp/db/data/analisis_precios2.csv", header=True)
